...
public class IXModPlugin extends BaseModPlugin implements SectorGeneratorPlugin {
	...
	public void beforeGameSave() {
		...
		boolean isIX = ("ix_battlegroup").equals(Misc.getCommissionFactionId());
	...
	private void initializeFaction(SectorAPI sector) {
		...
		Global.getSector().getPlayerMemoryWithoutUpdate().set("$reputationIsSetIX", false);
		
		//reputations set by IXReputationListener.setIXHostileToAll() in beforeGameSave
		/*
	...
	public void onGameLoad(boolean newGame) {
		...
		//hotfix for marzanna to commissioned ix player rep = 0, can be deleted after next patch
		if (sector.getFaction("ix_marzanna") == null || sector.getFaction("ix_battlegroup") == null) return;
		...
		if (ixRep != marRep) sector.getFaction("ix_marzanna").getRelToPlayer().setRel(ixRep);
	}
