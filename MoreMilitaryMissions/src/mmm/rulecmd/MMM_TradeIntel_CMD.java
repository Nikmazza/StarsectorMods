...
import mmm.Utils;
import mmm.missions.MyBreadcrumbIntel;
...
public class MMM_TradeIntel_CMD extends BaseCommandPlugin {
    ...
    // Settings
    // Chance that a scavenger wants to trade before reaching the ruins system.
...
//    public static final float MAX_SEARCH_LY = 1000f;
    ...
    public static final String TARGET_ID = "$mmm_tradeIntel_targetId";
    ...
    // Returns all the entities with BreadcrumbIntel.
    public static Set<SectorEntityToken> getBreadcrumbIntelTargets() {
        ...
        for (IntelInfoPlugin curr : Global.getSector().getIntelManager().getIntel(MyBreadcrumbIntel.class)) {
            if (curr instanceof MyBreadcrumbIntel) {
                MyBreadcrumbIntel intel = (MyBreadcrumbIntel) curr;
                result.add(intel.getRemoveTrigger());
            }
        }
...
//                    log.debug("Candidate planet: " + data);
    ...
    public boolean execute(String ruleId, InteractionDialogAPI dialog, List<Misc.Token> params,
        ...
        if (!params.isEmpty() && params.get(0).getString(memoryMap).equals("accept")) {
...
//            }
            MyBreadcrumbIntel intel = new MyBreadcrumbIntel(data.target, data.subject,
                    "You purchased intel about " + data.fullDesc);
