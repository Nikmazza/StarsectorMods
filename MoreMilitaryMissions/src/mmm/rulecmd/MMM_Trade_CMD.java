...
public class MMM_Trade_CMD extends BaseCommandPlugin implements InteractionDialogPlugin {
    ...
    public static final float TARIFF = 0.3f;
    ...
    public static PickedCommodities pickCommodities(SectorEntityToken fleet) {
        ...
        for (CommoditySpecAPI comm : settings.getAllCommoditySpecs()) {
            if (comm.isPersonnel() || comm.isMeta() || comm.isNonEcon() || comm.getBasePrice() < 1f) continue;
    ...
    public static TradeLimits computeTradeLimits(SectorEntityToken fleet, MemoryAPI memory,
        ...
        for (String buy_id : commodities.buy_ids) {
            ...
            for (String sell_id : new_sell_ids) {
                ...
                float exchanged_limit = buy_limit * buy_comm.getBasePrice() / sell_comm.getBasePrice() / (1 - TARIFF);
    ...
    public class TradeCargoPickerListener implements CargoPickerListener {
        ...
        public void pickedCargo(CargoAPI cargo) {
            ...
            int bought = (int) Math.floor(value * (1 - TARIFF) / buy_comm.getBasePrice());
...
//        log.debug(MessageFormat.format("optionSelected optionData={0}, this={1}", optionData, this));
    ...
    public boolean execute(String ruleId, InteractionDialogAPI dialog, List<Misc.Token> params,
        ...
        if (!params.isEmpty() && params.get(0).getString(memoryMap).equals("checkShip")) {
            MemoryAPI memory = dialog.getInteractionTarget().getMemoryWithoutUpdate();
            if (memory == null) return false;  // Sanity check

            final List<String> SCAVENGER_TYPES = Arrays.asList(
                    FleetTypes.SCAVENGER_SMALL, FleetTypes.SCAVENGER_MEDIUM, FleetTypes.SCAVENGER_LARGE);
            return memory.getBoolean(MemFlags.MEMORY_KEY_SCAVENGER) ||
                    SCAVENGER_TYPES.contains(memory.getString(MemFlags.MEMORY_KEY_FLEET_TYPE));
        }
