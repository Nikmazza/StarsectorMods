...
public class MMM_NGC_CMD extends BaseCommandPlugin implements InteractionDialogPlugin {
    ...
    public static List<FactionAPI> getAllFactions(boolean has_intel) {
        ...
        for (FactionAPI faction : Global.getSector().getAllFactions()) {
            ...
            // We iterate through all ship roles because there's no FactionAPI.getKnownVariants method.
            for (String role : Utils.allShipRoles()) {
                if (!faction.getVariantsForRole(role).isEmpty()) {
                    result.add(faction);
                    continue factionLoop;
                }
            }
        }
        Collections.sort(result, Comparator.comparing(FactionAPI::getDisplayName));
        return result;
    }
