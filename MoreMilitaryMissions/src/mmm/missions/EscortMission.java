...
public class EscortMission extends OrbitalMissionBase implements FleetEventListener {
    ...
    public List<CampaignFleetAPI> pirateFleets = new ArrayList<>();
...
//        public long last_print_ts = Long.MIN_VALUE;  // Not used
    ...
    public class PirateFleetSpawner implements ConditionChecker, TriggerAction {
        ...
        public void doAction(TriggerActionContext context) {
            ...
            CreateFleetResult result = createFleet(market.getPrimaryEntity(), "Convoy Raider", Factions.PIRATES,
                    Factions.PIRATES, 3, enemy_fp, .77f);
            ...
            if (result.fleet == null) {
                log.error(MessageFormat.format("Failed to create fleet with {0} strength.", enemy_str));
                return;
            }

            final CampaignFleetAPI fleet = result.fleet;
            final float defeatThreshold = fleet.getEffectiveStrength() * FLEET_DEFEAT_RATIO;
            pirateFleets.add(fleet);

            fleet.getContainingLocation().removeEntity(fleet);
            Global.getSector().getHyperspace().addEntity(fleet);
            fleet.setLocation(closest_location.x, closest_location.y);
            fleet.clearAssignments();

            Script onCompletion = new Script() {
                @Override
                public void run() {
                    // If the pirate fleet isn't defeated, have it try to intercept the convoy again.
                    if (route.getActiveFleet() != null && !isEnded() && !isEnding() &&
                            fleet.getEffectiveStrength() > defeatThreshold) {
//                        log.debug("onCompletion: INTERCEPT");
                        fleet.addAssignment(FleetAssignment.STANDING_DOWN, null,
                                0.5f + 0.5f * (float) Math.random());
                        fleet.addAssignment(FleetAssignment.INTERCEPT, route.getActiveFleet(), 30f,
                                this);
                    } else {
                        addDespawnAssignment(fleet, true);
                    }
                }
            };

            fleet.addAssignment(FleetAssignment.INTERCEPT, route.getActiveFleet(), 20f, onCompletion);

            // Not sure if these 2 flags does anything.
            setFlag(fleet, MemFlags.MEMORY_KEY_ALLOW_LONG_PURSUIT, false);
            setFlag(fleet, MemFlags.MEMORY_KEY_MAKE_AGGRESSIVE, false);
            // Make sure the pirate fleet can catch up to the convoy.
            modifyBurnSpeed(fleet, route.getActiveFleet(), 1);
        }
    }
    ...
    // Determine whether the provided faction is a high hostility faction. It's defined as the top 25% of factions
    // by number of hostile factions, that is also hostile to at least 5 factions. In vanilla this works out to be
    // Pirates and Path.
    public static boolean isHighHostilityFaction(FactionAPI faction) {
        HashSet<FactionAPI> factionsWithMarkets = new HashSet<>();
        for (MarketAPI market : Global.getSector().getEconomy().getMarketsCopy()) {
            if (market.getFaction().isShowInIntelTab()) {
                factionsWithMarkets.add(market.getFaction());
            }
        }

        ArrayList<Pair<FactionAPI, Integer>> factionsByHostility = new ArrayList<>();
        for (FactionAPI curr : factionsWithMarkets) {
            int hostility = 0;
            for (FactionAPI other : factionsWithMarkets) {
                if (curr.isHostileTo(other)) {
                    ++hostility;
                }
            }
            factionsByHostility.add(new Pair<>(curr, hostility));
        }

        // Sort factions by hostility descending.
        Collections.sort(factionsByHostility, new Comparator<Pair<FactionAPI, Integer>>() {
            @Override
            public int compare(Pair<FactionAPI, Integer> o1, Pair<FactionAPI, Integer> o2) {
                return o2.two - o1.two;
            }
        });

//        for (Pair<FactionAPI, Integer> pair : factionsByHostility) {
//            log.debug(MessageFormat.format("isHighHostilityFaction: {0} is hostile to {1}/{2} factions ",
//                    pair.one.getDisplayName(), pair.two, factionsWithMarkets.size()));
//        }

        int limit = Math.round(factionsWithMarkets.size() * 0.25f);
        for (Pair<FactionAPI, Integer> pair : factionsByHostility.subList(0, limit)) {
            if (faction == pair.one && pair.two >= 5) {
                return true;
            }
        }
...
//                    econ_route_data.to == null || econ_route_data.smuggling || econ_route_data.size <= 3 ||
                    econ_route_data.to == null || econ_route_data.smuggling ||
            ...
            FactionAPI faction = Global.getSector().getFaction(route.getFactionId());
            if (faction.getRelToPlayer().isAtBest(RepLevel.SUSPICIOUS)) {
                continue;
            }
            ...
            // Only allow size 3 or better, unless it's a high hostility faction or your own faction.
            if (econ_route_data.size <= 3 && !faction.isPlayerFaction() && !isHighHostilityFaction(faction)) continue;
...
//                log.debug("No eligible trade routes for person " + person.getNameString());
        ...
        EconomyRouteData econ_route_data = (EconomyRouteData) route.getCustom();
        boolean sameSystem = econ_route_data.to.getStarSystem() == createdAt.getStarSystem();

        FactionAPI faction = createdAt.getFaction();
        boolean isHighHostility = isHighHostilityFaction(faction);

        // Compute what commodities we want to carry.
        delivery_data = computeDeliveries(false, sameSystem, isHighHostility);
        return_data = computeDeliveries(true, sameSystem, isHighHostility);
        ...
        if (delivery_data != null || return_data != null) {
            ...
            if (factionId.equals(Factions.PIRATES)) {
                ...
                agent = findOrCreateTrader(factionId, econ_route_data.to,
                        true);
            }
...
//        boolean is_valuable = false;
        ...
        // See if factions is at war
        boolean hasHostility = false;
        for (IntelInfoPlugin plugin : Global.getSector().getIntelManager().getIntel(FactionHostilityIntel.class)) {
            ...
            if (intel.getOne().getId().equals(factionId) || intel.getTwo().getId().equals(factionId)) {
                hasHostility = true;
                break;
            }
        }
        ...
        if (isHighHostility || hasHostility) {
        ...
        if (sameSystem) {
            ...
            setCreditReward(myGetRoundNumber(base_credits / 2));
            return true;
        } else if (isHighHostility) {
            base_credits *= 2;
        }

        // Increase reward based on distance in LY and player fleet FP
        ...
        int estimatedPirateFp = Math.round(econ_route_data.size * 25 * MAX_PIRATE_FP_RATIO);
        if (distance_ly >= PIRATE_SPAWN_LY) {
            int allowedPlayerFp = Math.min(estimatedPirateFp, getPlayerFleetStrength());
            fp_credits = Math.min(allowedPlayerFp * CREDITS_PER_PLAYER_FP, base_credits);
            ...
            if (faction.isAtBest(Factions.PIRATES, RepLevel.HOSTILE)) {
                for (Stage stage : Arrays.asList(Stage.DEPARTURE, Stage.RETURN)) {
                    if (!Global.getSettings().isDevMode() && getGenRandom().nextFloat() >= PIRATE_SPAWN_CHANCE) continue;
        ...
        int ly_credits = Math.min(base_credits,
                Math.round(distance_ly * CREDIT_RATIO_PER_LY * (base_credits + fp_credits)));
        ...
        log.info(MessageFormat.format(
                "setCreditReward: credits={0}, base_credits={1}, fp_credits={2}, ly_credits={3}, " +
                        "isHighHostility={4}, distance_ly={5}, estimatedPirateFp={6}",
                credits, base_credits, fp_credits, ly_credits, isHighHostility, distance_ly, estimatedPirateFp));
        return true;
    }
...
//        EconomyRouteData econ_route_data = (EconomyRouteData) route.getCustom();
    ...
    protected void endFailureImpl(InteractionDialogAPI dialog, Map<String, MemoryAPI> memoryMap) {
        for (CampaignFleetAPI fleet : pirateFleets) {
            addDespawnAssignment(fleet, false);
        }
    }

    @Override
    protected void endSuccessImpl(InteractionDialogAPI dialog, Map<String, MemoryAPI> memoryMap) {
        // Assuming defaultPotentialContactProbability is 0.25f, and bar giver has 0 relation before mission (thus
        // 5% on success).  This gives player faction 50% chance, and non-player 40%.
        // 40% non-player, 50% player
        addPotentialContact(this, dialog, 0.79f, 0.62f);
    ...
    public DeliveryData computeDeliveries(boolean is_return, boolean sameSystem, boolean isHighHostility) {
        EconomyRouteData econ_route_data = (EconomyRouteData) route.getCustom();
        ...
        for (CargoQuantityData curr : sorted) {
            ...
            ++size;
            if (spec.getId().equals(Commodities.SHIPS) || spec.getId().equals(Commodities.CREW)) continue;
...
//                Arrays.toString(limits), Arrays.toString(commodities_by_type), commodities));
        ...
        for (Map.Entry<String, Integer> entry : commodities.entrySet()) {
            ...
            // halve delivery reward if same system; increase it if high hostility faction (pirate/path)
            if (sameSystem) {
                delivery_reward /= 2;
            } else if (isHighHostility) {
                delivery_reward *= 1.5f;
            }
...
//            plugin.execute(null, dialog, Arrays.asList(new Token(id, TokenType.LITERAL)), null);
    ...
    public void reportBattleOccurred(CampaignFleetAPI fleet, CampaignFleetAPI primaryWinner, BattleAPI battle) {
        ...
        if (!fleet.isAlive()) {
            stage_description = getFleetTypeName() + " is destroyed.";
        } else if (!battle.isPlayerInvolved() && previous_size > fleet_size) {
            log.info(MessageFormat.format("Player fleet is missing in action; previous_size={0}, fleet_size={1}",
                    previous_size, fleet_size));
            stage_description = getFleetTypeName() + " lost ships while player is missing in action.";
        } else {
            ...
            if (lost) {
                stage_description = MessageFormat.format("{0} lost too much cargo ({1}%).",
    ...
    protected void addBulletPoints(TooltipMakerAPI info, ListInfoMode mode) {
        ...
        if (steps == 1) {  // Stage update
            ...
            if (Arrays.asList(Stage.DST_WAIT, Stage.SRC_WAIT, Stage.ROUTE_CANCELLED).contains(current_stage)) {
                for (PersonAPI person : Arrays.asList(agent, getPerson())) {
                    ...
                    if (memory.getBoolean(NEEDS_COMMODITIES)) {
                        pattern = "Talk to {0} to unload commodities within {1} {2}{3}.";
                    } else if (memory.getBoolean(HAS_COMMODITIES) && current_stage != Stage.ROUTE_CANCELLED) {
                        pattern = "Optional: talk to {0} to acquire commodities within {1} {2}{3}.";
                    }
                    if (pattern != null) {
                        boolean arrived;
                        if (person == agent) {
                            arrived = current_stage == Stage.DST_WAIT;
                        } else {
                            arrived = current_stage == Stage.SRC_WAIT;
                        }
                        String ofArrival = arrived ? "" : " of arrival";

                        info.addPara(MessageFormat.format(pattern, person.getNameString(), days_str, day_or_days, ofArrival),
            ...
            if (current_stage != Stage.SRC_WAIT && current_stage != Stage.ROUTE_CANCELLED) {
                info.addPara("Participate in all fleet engagements and protect cargo.",
                        text_color, 0f);

                String daysStr = MessageFormat.format("{0} days", MAX_DAYS_LAST_SEEN);
                ArrayList<String> highlights = new ArrayList<>();
                highlights.add(daysStr);
                ...
                if (last_seen_checker != null) {
                    ...
                    if (last_seen_ts < now) {
                        String lastSeenDays = getDaysFromNowStr(last_seen_ts, now);
                        highlights.add(lastSeenDays);
                        lastSeenStr = MessageFormat.format(" (last seen {0} ago)", lastSeenDays);
                    }
                }
                info.addPara(MessageFormat.format(
                        "Do not leave the convoy''s sensor range for more than {0}{1}.",
                                daysStr, lastSeenStr),
                        0f, text_color, h,  highlights.toArray(new String[0]));
            }
