...
public abstract class OrbitalMissionBase extends HubMissionWithBarEvent {
    ...
    // TODO: store at different memory location per mission
    ...
    public static void updateSeed(String missionId, MarketAPI market, PersonAPI person) {
        if (market == null) return;  // Should not happen except with mods that do funky things
        Map<String, Long> mapping = getSeedMap(market);
        mapping.put(getSeedKey(missionId, person), getNewSeed(missionId, market, person));
    }

    public static long getSeed(String missionId, MarketAPI market, PersonAPI person) {
        Map<String, Long> mapping = getSeedMap(market);
    ...
    public static Random getRandom(String missionId, MarketAPI market, PersonAPI person) {
        return new Random(getSeed(missionId, market, person));
    }
...
//                    commander.getTags());
    ...
    public static int getFleetStrength(CampaignFleetAPI fleet) {
        ...
        // Compatibility for 0.98a-RC5 since fleet.getFleetPoints() and fleet.getEffectiveStrength() where it sometimes
        // returns 0.
        float memberFp = 0;
        float memberFp2 = 0;
        for (FleetMemberAPI member : fleet.getMembersWithFightersCopy()) {
            memberFp += member.getMemberStrength();
            memberFp2 = Misc.getMemberStrength(member);
        }

        float maxStrength = Math.max(memberFp2, Math.max(memberFp, fleet.getEffectiveStrength()));
        float points = Math.round(maxStrength * strength_ratio);
        return Math.round(fleet.isStationMode() ? points * STATION_EFFECTIVE_FP_RATIO : points);
    }
    // If reinforcement_faction is set, tags the fleet with STRENGTH_RATIO_KEY so it can be applied and then calls above.
    // Otherwise returns the vanilla fleet effective strength.
    public static int getFleetStrength(CampaignFleetAPI fleet, String reinforcement_faction) {
        if (reinforcement_faction != null) {
        ...
        return Math.round(fleet.getEffectiveStrength());
    }
    ...
    public static CreateFleetResult createFleet(
        ...
        float max_input_fp_per_output = 1.5f;  // Some sanity limits
        Float strength_ratio = FACTION_STRENGTH_RATIO.get(reinforcement_faction);
        ...
        for (int i = 0; i < attempts; ++i){
            ...
            CampaignFleetAPI fleet;
            try {
                // setting MagicFleetBuilder.getFleetParams().random still results in different fleets
            ...
            fleet.forceSync();
            float minBurn = fleet.getFleetData().getMinBurnLevel();
            ...
            log.debug(MessageFormat.format(
                    "createFleet: reinforcement={0}, expected_fp={1}, input_fp={2}, output_fp={3}({4} points)" +
                            ", input_fp_per_output={5}(max {6}), threshold={7}, minBurn={8}, attempt {9}/{10}",
                    reinforcement_faction, expected_fp, input_fp, output_fp, fleet.getFleetPoints(),
                    input_fp_per_output, max_input_fp_per_output, threshold, minBurn, i + 1, attempts));

            // Don't use fleets with 0 burn
            if (minBurn < 1f) {
...
//        log.debug("applyTradeMods on market " + market.getName());
    ...
    public static void addDespawnAssignment(CampaignFleetAPI fleet, boolean clearAssignments) {
        if (fleet == null || !fleet.isAlive()) return;

        SectorEntityToken target = Misc.findNearestJumpPoint(fleet);
        if (target == null) return;  // Sanity check
//        log.debug(MessageFormat.format("addDespawnAssignment: fleet={0}, target={1}, curLoc={2}", fleet.getFullName(),
//                target.getName(), target.getLocation()));
        ...
        String actionText = fleet.isInHyperspace() ? null : "leaving system";
    ...
    // Inflates the fleet and keep it from being deflated, as fleet inflation might triggers code paths that's undesirable.
    public static void keepFleetInflated(CampaignFleetAPI fleet) {
        fleet.inflateIfNeeded();
        for (FleetMemberAPI member : fleet.getMembersWithFightersCopy()) {
            if (member != null && member.getVariant() != null) {
                member.getVariant().setOriginalVariant(null);
            }
        }
    }

    public static void acceptCommon(String missionId, PersonAPI person, MarketAPI market) {
        updateSeed(missionId, market == null ? person.getMarket() : market, person);
    }

    @Override
    public void acceptImpl(InteractionDialogAPI dialog, Map<String, MemoryAPI> memoryMap) {
        super.acceptImpl(dialog, memoryMap);
        acceptCommon(getMissionId(), getPerson(), null);
    }

    // Sort a fleet; prioritizing flagship, non-civilian ships, larger hulls, and break ties with the original ship order.
    // Also remove fighters.
    public static List<FleetMemberAPI> sortedFleet(List<FleetMemberAPI> members) {
        TreeMap<Integer, FleetMemberAPI> mapping = new TreeMap<>();
        for (int i = 0; i < members.size(); ++i) {
            ...
            priority -= variant.getHullSize().ordinal() * (Integer.MAX_VALUE / 64);

            mapping.put(priority, member);
        }
