...
public class TemporalLockStats extends BaseShipSystemScript {
	public static Object KEY_SHIP = new Object();
	public static Object KEY_TARGET = new Object();
	
	public static float MAX_TIME_MULT = 0.5f;
	...
	public void apply(MutableShipStatsAPI stats, final String id, State state, float effectLevel) {
		...
		final String targetDataKey = ship.getId() + "_lock_target_data";
		...
		if (state == State.IN && targetDataObj == null) {
			...
			if (target != null) {
				if (target.getFluxTracker().showFloaty() || 
					...
					target.getFluxTracker().showOverloadFloatyIfNeeded("Locked Time!", TEXT_COLOR, 4f, true);
					ship.getFluxTracker().showOverloadFloatyIfNeeded("Locked Time!", TEXT_COLOR, 4f, true);
				}
			}
		...
		targetData.currTimeMult = MAX_TIME_MULT * effectLevel;
		ship.getMutableStats().getTimeMult().modifyMult(id, MAX_TIME_MULT);
				...
				public void advance(float amount, List<InputEventAPI> events) {
					...
					if (targetData.target == Global.getCombatEngine().getPlayerShip()) { 
						Global.getCombatEngine().maintainStatusForPlayerShip(KEY_TARGET, 
								...
								"Time slowed by" + (int)((targetData.currTimeMult - 1f) * 100f) + "%", true);
					}
					
					if (targetData.currTimeMult <= 1f || !targetData.ship.isAlive()) {
						...
						targetData.target.getMutableStats().getHullDamageTakenMult().modifyMult(id, targetData.currTimeMult);
						targetData.target.getMutableStats().getArmorDamageTakenMult().modifyMult(id, targetData.currTimeMult);
						targetData.target.getMutableStats().getShieldDamageTakenMult().modifyMult(id, targetData.currTimeMult);
						targetData.target.getMutableStats().getEmpDamageTakenMult().modifyMult(id, targetData.currTimeMult);
					}
				}
			};
	...
	public StatusData getStatusData(int index, State state, float effectLevel) {
		if (effectLevel > 0) {
			if (index == 0) {
				...
				return new StatusData("Time slowed by" + (int)((timeMult - 1f) * 100f) + "%", true);
			}
		}
		return null;
	}
