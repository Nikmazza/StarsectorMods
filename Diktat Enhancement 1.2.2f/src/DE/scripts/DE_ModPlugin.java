...
public class DE_ModPlugin extends BaseModPlugin {
    ...
    private static void initTriTachMarketsYma() {
        ...
        PlanetAPI khuno = system.addPlanet("khuno", system.getEntityById("yma"), "Khuno", "frozen", 280, 100, 11000, 700);
        ...
        // gravity well
        //Global.getLogger(DE_ModPlugin.class).info("code working");
        LocationAPI hyper = Global.getSector().getHyperspace();
        //Global.getLogger(DE_ModPlugin.class).info("well made");
        NascentGravityWellAPI well = Global.getSector().createNascentGravityWell(khuno, 100f); //100f
        //Global.getLogger(DE_ModPlugin.class).info("well updated");
        well.autoUpdateHyperLocationBasedOnInSystemEntityAtRadius(khuno, 10f);
        //Global.getLogger(DE_ModPlugin.class).info("well added");
    ...
    private static void initTriTachMarketsAztlan() {
        ...
        PlanetAPI qui = system.addPlanet("qui", system.getEntityById("aztlan"), "Itztlacoliuhqui", "frozen", 280, 140, 6000, 400);
        ...
        // gravity well
        LocationAPI hyper = Global.getSector().getHyperspace();
        NascentGravityWellAPI well = Global.getSector().createNascentGravityWell(qui, 140f);
        well.autoUpdateHyperLocationBasedOnInSystemEntityAtRadius(qui, 10f);
        hyper.addEntity(well);
    }
    ...
    private static void initTriTachMarketsZagan() {
        ...
        PlanetAPI kukayin = system.addPlanet("kukayin", system.getEntityById("zagan"), "Kukayin", "tundra", 280, 160, 7000, 300);
        ...
        // gravity well
        LocationAPI hyper = Global.getSector().getHyperspace();
        NascentGravityWellAPI well = Global.getSector().createNascentGravityWell(kukayin, 160f);
        well.autoUpdateHyperLocationBasedOnInSystemEntityAtRadius(kukayin, 10f);
        hyper.addEntity(well);
    }
    ...
    private static void initTriTachMarketsHybrasil() {
        ...
        // gravity well
        LocationAPI hyper = Global.getSector().getHyperspace();
        NascentGravityWellAPI well = Global.getSector().createNascentGravityWell(cailleach, 150f);
        well.autoUpdateHyperLocationBasedOnInSystemEntityAtRadius(cailleach, 10f);
        hyper.addEntity(well);
        ...
        // gravity well
        NascentGravityWellAPI well2 = Global.getSector().createNascentGravityWell(bheur, 150f);
        well2.autoUpdateHyperLocationBasedOnInSystemEntityAtRadius(bheur, 10f);
    ...
    private static void initTriTachMarketsTyle() {
        ...
        PlanetAPI aisoyim = system.addPlanet("aisoyim", system.getEntityById("tyle"), "Aisoyimstan", "rocky_ice", 280, 140, 7000, 300);
        ...
        // gravity well
        LocationAPI hyper = Global.getSector().getHyperspace();
        NascentGravityWellAPI well = Global.getSector().createNascentGravityWell(aisoyim, 140f);
        well.autoUpdateHyperLocationBasedOnInSystemEntityAtRadius(aisoyim, 10f);
        hyper.addEntity(well);
    }
        ...
        if (!haveNexerelin || SectorManager.getManager().isCorvusMode()) {
            ...
            if (DEdisablelobers) {
                ...
                for (String s : Arrays.asList("de_nephrops_wing", "de_metanephrops_wing", "de_homarus_wing")) {
                    ...
                    Global.getSector().getFaction("sindrian_diktat").removePriorityFighter(s);
                }
                Global.getSector().getFaction("lions_guard").removePriorityFighter("de_homarus_wing");
    ...
    private static void initAskonia() {
        ...
        if (!isPAGSM) {
            ...
            // Lobster… I will cut you down, break you apart,
            if (DEdisablelobers) {
                for (String s : Arrays.asList("no_drop", "no_sell", "no_bp_drop", "no_dealer", "restricted", "no_bp")) {
                    Global.getSettings().getFighterWingSpec("de_nephrops_wing").addTag(s);
                    Global.getSettings().getFighterWingSpec("de_metanephrops_wing").addTag(s);
    ...
    private static void initFortressModeChanges() {
        ...
        NascentGravityWellAPI well = Global.getSector().createNascentGravityWell(drakon, 150f);
        well.autoUpdateHyperLocationBasedOnInSystemEntityAtRadius(drakon, 470f);
        hyper.addEntity(well);

        // forgot the wells of every other market im sobbing
        SectorEntityToken midgard = sector.getStarSystem("Valhalla").getEntityById("midgard");
        
        NascentGravityWellAPI well2 = Global.getSector().createNascentGravityWell(midgard, 170f);
        well2.autoUpdateHyperLocationBasedOnInSystemEntityAtRadius(midgard, 10f);
        hyper.addEntity(well2);
        ...
        // Lion's Cubs revival
            ...
            if (!isPAGSM) {
                ...
                // I will grind you down until VOLTURN-Y HIMSELF CRIES FOR MERCY!
                if (DEdisablelobers) {
                    for (String s : Arrays.asList("no_drop", "no_sell", "no_bp_drop", "no_dealer", "restricted", "no_bp")) {
                        Global.getSettings().getFighterWingSpec("de_nephrops_wing").addTag(s);
                        Global.getSettings().getFighterWingSpec("de_metanephrops_wing").addTag(s);
                ...
                // My code shall RELISH ENDING YOU... HERE! AND! NOW!
                if (DEdisablelobers) {
                    for (String s : Arrays.asList("no_drop", "no_sell", "no_bp_drop", "no_dealer", "restricted", "no_bp")) {
                        Global.getSettings().getFighterWingSpec("de_nephrops_wing").addTag(s);
                        Global.getSettings().getFighterWingSpec("de_metanephrops_wing").addTag(s);
    ...
    public void onGameLoad (boolean newGame){
        ...
        // removing lobers 2.0 - Wizzball said putting this here might purge them once and for all, those buggers
        if (DEdisablelobers) {
            for (String s : Arrays.asList("no_drop", "no_sell", "no_bp_drop", "no_dealer", "restricted", "no_bp")) {
                Global.getSettings().getFighterWingSpec("de_nephrops_wing").addTag(s);
                Global.getSettings().getFighterWingSpec("de_metanephrops_wing").addTag(s);
