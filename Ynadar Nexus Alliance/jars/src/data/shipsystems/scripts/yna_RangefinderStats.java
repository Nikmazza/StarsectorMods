...
public class yna_RangefinderStats extends BaseShipSystemScript {
	...
	protected List<RFData> dishData = new ArrayList<yna_RangefinderStats.RFData>();
...
//			}
	...
	public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
		...
		lidarRange += 100f;
		//stats.getBeamWeaponRangeBonus().modifyFlat("yna_rfind", lidarRange);  // Alex, WHY?
...
//		stats.getEnergyWeaponRangeBonus().unmodifyPercent(id);
		...
		stats.getBeamWeaponDamageMult().unmodifyMult(id);
	...
	public StatusData getStatusData(int index, State state, float effectLevel) {
		...
		if (index == 4) {
			return new StatusData("weapon range +" + (int) D_RANGE + "%", false);
		}
		if (index == 3) {
	...
	public String getDisplayNameOverride(State state, float effectLevel) {
		if (state == State.IDLE || state == State.COOLDOWN) {
			return "Rangefinder Pulse - Passive";
		}
		return null;
	}
	*/
