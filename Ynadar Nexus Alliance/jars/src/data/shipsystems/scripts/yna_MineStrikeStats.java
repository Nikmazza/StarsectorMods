...
public class yna_MineStrikeStats extends BaseShipSystemScript implements MineStrikeStatsAIInfoProvider {
	
	protected static float MINE_RANGE = 1500f;
	
	public static final float MIN_SPAWN_DIST = 100f;
	public static final float MIN_SPAWN_DIST_FRIGATE = 150f;
	...
	public void spawnMine(ShipAPI source, Vector2f mineLoc) {
		...
		Vector2f currLoc2 = Misc.getPointAtRadius(mineLoc, 50f + (float) Math.random() * 75f);
		Vector2f currLoc3 = Misc.getPointAtRadius(mineLoc, 50f + (float) Math.random() * 100f);
		...
		if (currLoc == null) {
			...
			currLoc2 = Misc.getPointAtRadius(mineLoc, 30f + (float) Math.random() * 40f);
			currLoc3 = Misc.getPointAtRadius(mineLoc, 30f + (float) Math.random() * 50f);
		}
		
		
		
		//Vector2f currLoc = mineLoc;
		MissileAPI mine = (MissileAPI) engine.spawnProjectile(source, null, "yna_minelayer", 
				currLoc, (float) Math.random() * 400f, null);
		MissileAPI mine2 = (MissileAPI) engine.spawnProjectile(source, null, "yna_minelayer", 
				currLoc2, (float) Math.random() * 400f, null);
		MissileAPI mine3 = (MissileAPI) engine.spawnProjectile(source, null, "yna_minelayer", 
				currLoc3, (float) Math.random() * 400f, null);
		...
		if (source != null) {
			...
			Global.getCombatEngine().applyDamageModifiersToSpawnedProjectileWithNullWeapon(
											source, WeaponType.MISSILE, false, mine2.getDamage());
			Global.getCombatEngine().applyDamageModifiersToSpawnedProjectileWithNullWeapon(
											source, WeaponType.MISSILE, false, mine3.getDamage());
		}
		...
		mine2.getVelocity().scale(0);
		mine2.fadeOutThenIn(fadeInTime);
		mine3.getVelocity().scale(0);
		mine3.fadeOutThenIn(fadeInTime);
		
		Global.getCombatEngine().addPlugin(createMissileJitterPlugin(mine, fadeInTime));
		Global.getCombatEngine().addPlugin(createMissileJitterPlugin(mine2, fadeInTime));
		Global.getCombatEngine().addPlugin(createMissileJitterPlugin(mine3, fadeInTime));
		
		//mine.setFlightTime((float) Math.random());
		float liveTime = LIVE_TIME;
		//liveTime = 0.01f;
		mine.setFlightTime(mine.getMaxFlightTime() - liveTime);
		mine2.setFlightTime(mine.getMaxFlightTime() - liveTime);
		mine3.setFlightTime(mine.getMaxFlightTime() - liveTime);
