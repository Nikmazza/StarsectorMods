    ...
    public String modifyDamageDealt(Object param, CombatEntityAPI target, DamageAPI damage, Vector2f point, boolean shieldHit) {
        ...
        if(param instanceof DamagingProjectileAPI && target instanceof ShipAPI){
            ...
            if(proj.getWeapon()!=null && proj.getWeapon().getSpec()!=null){
                if(proj.getWeapon().getSpec().getWeaponId().equals("ssp_blaster")
                        ||proj.getWeapon().getSpec().getWeaponId().equals("ssp_BigFackingGPD")){
                    //从这里才算正式开始
                    if(HaHaHaHaHashmap.get(target) != null && target instanceof ShipAPI){
                        ...
                        damage.getModifier().modifyMult(id,1+HitTimes*0.02f);
                        return id;
                    }
