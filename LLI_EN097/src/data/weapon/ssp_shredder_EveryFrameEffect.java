...
public class ssp_shredder_EveryFrameEffect implements EveryFrameWeaponEffectPlugin {
    ...
    protected IntervalUtil interval = new IntervalUtil(0.1f, 0.5f);
...
//            for(float beam_length=0;beam_length<beam.getLengthPrevFrame();beam_length+=20f+MathUtils.getRandomNumberInRange(-10,4)){
//                Vector2f Loc =MathUtils.getPoint(FP,beam_length,beam.getWeapon().getCurrAngle());
//                Global.getCombatEngine().addNebulaParticle(Loc,beam.getSource().getVelocity(),20f,2f,0.25f,1f,0.125f, beam.getFringeColor());
//                Global.getCombatEngine().addNegativeNebulaParticle(Loc,beam.getSource().getVelocity(),45f,1f,0.25f,1f,0.125f, SSPMisc.Anti_Color(beam.getFringeColor()));
...
//            }
            ...
            //沿光束移动直到光束终点的电弧效果
            if (T>0f) {
                Global.getCombatEngine().spawnEmpArcVisual(beam.getFrom(),weapon.getShip(), MathUtils.getPoint(weapon.getLocation(), Math.min(beam.getWeapon().getRange() * (T / beam.getWeapon().getSpec().getBurstDuration()), beam.getLengthPrevFrame()), Misc.getAngleInDegrees(weapon.getLocation(), beam.getTo())),weapon.getShip(), 5, beam.getFringeColor(), beam.getCoreColor());
            }
        }
      //产生电弧
