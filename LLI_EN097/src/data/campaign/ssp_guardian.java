...
public class ssp_guardian extends BaseIndustry implements RouteFleetSpawner, FleetEventListener {
	...
	public CampaignFleetAPI spawnFleet(RouteData route) {
		...
		String fleetFactionType = "LLI";
		switch (type) {
		case FAST:
			fleetFactionType="LLI_elite";
			combat = Math.round(5f + (float) random.nextFloat() * 2f) * 5f;
			break;
		case COMBAT:
			fleetFactionType="LLI_guardian";
			combat = Math.round(10f + (float) random.nextFloat() * 3f) * 5f;
			break;
		case HEAVY:
			fleetFactionType="LLI_legion";
		...
		FleetParamsV3 params = new FleetParamsV3(
				...
				fleetType,
				combat, // combatPts
				Math.round((float) random.nextFloat()) * 5f, // freighterPts
				Math.round((float) random.nextFloat()) * 5f, // tankerPts
				0f, // transportPts
				0f, // linerPts
				0f, // utilityPts
