...
public class LLICMD_BuyWaveM extends BaseCommandPlugin {
    public boolean execute(String ruleId, InteractionDialogAPI dialog, List<Misc.Token> params, Map<String, MemoryAPI> memoryMap) {
        ...
        if(Global.getSector().getPlayerFleet().getCargo().getCredits().get()>=1000000){
            FleetMemberAPI member= new FleetMember(1,"ssp_superdestoryer_standard", FleetMemberType.SHIP);
            Global.getSector().getPlayerFleet().getFleetData().addFleetMember(member);
            addShipGainText(member, dialog.getTextPanel());
            Global.getSector().getPlayerFleet().getCargo().getCredits().add(-1000000);
        }else{
