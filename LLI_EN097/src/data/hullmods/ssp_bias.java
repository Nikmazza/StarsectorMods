...
public class ssp_bias extends BaseHullMod {
    ...
    public void applyEffectsAfterShipCreation(ShipAPI ship, String id) {
        if(ship.getVariant().hasHullMod("ssp_T_beam")){HAS_HIGH_SCATTER_AMP=true;}else{HAS_HIGH_SCATTER_AMP=false;}
    ...
    public static class ssp_bias_EffectMod implements WeaponRangeModifier {
        ...
        public float getWeaponRangeFlatMod(ShipAPI ship, WeaponAPI weapon){
            float RANGE=0f;
            if(!ship.getVariant().hasHullMod("ssp_T_beam")){
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        LabelAPI label = tooltip.addPara(
                ...
                opad, h, Global.getSettings().getHullModSpec("ssp_T_beam").getDisplayName());
        label.setHighlight(Global.getSettings().getHullModSpec("ssp_T_beam").getDisplayName());
