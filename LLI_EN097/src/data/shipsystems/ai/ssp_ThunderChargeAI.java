...
public class ssp_ThunderChargeAI implements ShipSystemAIScript {
    ...
    public void advance(float amount, Vector2f missileDangerDir, Vector2f collisionDangerDir, ShipAPI target) {
        ...
        for( MissileAPI m : AIUtils.getNearbyEnemyMissiles(ship,1000f)){
            if(m == null) continue;
            if(m.getCollisionClass()==CollisionClass.NONE) continue;
        ...
        //防止过载自己
        if(ship.getVariant().hasHullMod("ssp_SYS_p")){
            if(ship.getMaxFlux()-ship.getFluxTracker().getCurrFlux() < 4000){return;}
        }else if(!ship.getVariant().hasHullMod("ssp_SYS_s") && !ship.getVariant().hasHullMod("ssp_SYS_p")){
