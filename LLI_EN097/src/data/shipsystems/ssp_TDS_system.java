...
public class ssp_TDS_system extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        if(!RunOnce){
            RunOnce=true;
            if(ship.getVariant().hasHullMod("ssp_SYS_p")){ship.getFluxTracker().increaseFlux(ship.getSystem().getFluxPerUse()*2,true);}
        }
        if(ship.getVariant().hasHullMod("ssp_SYS_s")){Mult=0f;}
...
//        }
    ...
    public float getRegenOverride(ShipAPI ship) {
        if(ship.getVariant().hasHullMod("ssp_SYS_p")){return 0.15f;}
        else return -1;
    }
    public int getUsesOverride(ShipAPI ship) {
        if(ship.getVariant().hasHullMod("ssp_SYS_p")){return 3;}
        else return -1;
    }
