...
public class ssp_microburn extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        if(stats.getVariant().hasHullMod("ssp_SYS_p")){ TurnRate_MULT=5f;  TurnAcceleration_MULT=1f; }
...
//            if (stats.getVariant().hasHullMod("ssp_SYS_p")){isdebuff =false; TurnRateString=SSPI18nUtil.getShipSystemString("ssp_microburn_FastTurnRate");}
...
//        stats.getMaxTurnRate().unmodify(id);
    ...
    public int getUsesOverride(ShipAPI ship) {
        if (ship != null) {
            if (ship.getVariant().hasHullMod("ssp_SYS_p")) { return 2;}
            if (ship.getVariant().hasHullMod("ssp_SYS_s")){return 4;}
        }
        return -1;
    }
    @Override
    public float getRegenOverride(ShipAPI ship) {
        if (ship != null) {
            if (ship.getVariant().hasHullMod("ssp_SYS_p")){return 0.10f;}}
        return -1;
    }
    ...
    public float getInOverride(ShipAPI ship) {
        if (ship != null) {
            if (ship.getVariant().hasHullMod("ssp_SYS_s")){return 0.6f;}}
        return -1;
    }
    @Override
