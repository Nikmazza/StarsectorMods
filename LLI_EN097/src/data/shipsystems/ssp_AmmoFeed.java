...
public class ssp_AmmoFeed extends BaseShipSystemScript {
	...
	public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
		...
		if (stats.getEntity() instanceof ShipAPI) {
			...
			return; }
		if(ship.getVariant().hasHullMod("ssp_SYS_p")){FC=2f;}
...
//						}
						if(weapon.getChargeLevel()==1){
							AmmoFeed_Point -= weapon.getSpec().getChargeTime()*(1-1/RoF_Mult);
							if (weapon.getCooldownRemaining()>0.1f){
								AmmoFeed_Point -= weapon.getCooldownRemaining()+weapon.getSpec().getChargeTime()*(1-1/RoF_Mult);
								weapon.setRemainingCooldownTo(0.1f);
							}
						}
						if(ship!= Global.getCombatEngine().getPlayerShip()|| (ship==Global.getCombatEngine().getPlayerShip()&&Global.getCombatEngine().getCombatUI().isAutopilotOn())){
							if (shouldattack && weapon.hasAIHint(WeaponAPI.AIHints.PD)){
	...
	public float getActiveOverride(ShipAPI ship) {
		if (ship != null) {
			if (ship.getVariant().hasHullMod("ssp_SYS_s")){return 8f;}
			if (ship.getVariant().hasHullMod("ssp_SYS_p")){return 14f;}
		}
		return -1;
	}
}
