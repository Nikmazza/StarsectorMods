 ...
 */
public class NES_DerelictSpawnScript {
	...
	static {
		SHIP_SPAWNS.add(new Pair<>("nes_fluorspar_d_Lancer", 1));
		SHIP_SPAWNS.add(new Pair<>("nes_fluorspar_d_Mothballed", 1));
		SHIP_SPAWNS.add(new Pair<>("nes_fluorspar_d_Laser", 1));
		SHIP_SPAWNS.add(new Pair<>("nes_fluorspar_d_Fire_Support", 1));
		SHIP_SPAWNS.add(new Pair<>("nes_hampter_hero_Strike", 1));
		SHIP_SPAWNS.add(new Pair<>("nes_hampter_hero_Artillery", 1));
	}
	 ...
	 */
	public static void spawnNESDerelicts (SectorAPI sector) {
			...
			while (numberOfSpawns < spawnData.two) {
				...
				while (placeToSpawn == null) {
					...
					if (system == null) {
						...
						LOGGER.info("No Nes teaser spawn found");
						return;
					}
				...
				addDerelict(system, spawnData.one, placeToSpawn.orbit, ShipRecoverySpecial.ShipCondition.PRISTINE, true, null);
				numberOfSpawns++;
				LOGGER.info("Spawned Nes teaser " + spawnData.one + " in " + system.getId());
			}
		}
	}
