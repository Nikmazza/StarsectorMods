...
public class jdp_mirrorshield extends BaseHullMod {
    ...
    public Color ZeroEntropyColor = new Color(255, 255, 255, 255);
    public Color FullEntropyColor = new Color(157, 255, 130, 255);
    ...
    public void advanceInCombat(ShipAPI ship, float amount) {
        ...
        if (customCombatData.get("jdp_MirrorInstability" + id) instanceof Float)
            Instability = (float) customCombatData.get("jdp_MirrorInstability" + id);

        ship.getShield().toggleOff();


        if (customCombatData.get("jdp_MirrorStart" + id) instanceof Float)
            ShieldStart = (float) customCombatData.get("jdp_MirrorStart" + id);

        if (customCombatData.get("jdp_MirrorUnbrake" + id) instanceof Float)
            StartUnbrake = (float) customCombatData.get("jdp_MirrorUnbrake" + id);
