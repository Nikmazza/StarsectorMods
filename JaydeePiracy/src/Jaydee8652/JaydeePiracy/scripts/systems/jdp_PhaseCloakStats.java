...
public class jdp_PhaseCloakStats extends BaseShipSystemScript {

	public static Color JITTER_COLOR = new Color(255, 50, 50,255);
	...
	public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
		...
		if (state == State.IN || state == State.ACTIVE) {
			...
			if (!isPhased) {
				...
				NegativeExplosionVisual.NEParams p = RiftCascadeMineExplosion.createStandardRiftParams(new Color(255,50,50,155), ship.getShieldRadiusEvenIfNoShield() * 0.25f);
				p.fadeOut = 0.15f;
				p.hitGlowSizeMult = 0.25f;
				p.underglow = new Color(255,50,50, 50);
				p.withHitGlow = false;
				p.noiseMag = 1.25f;
