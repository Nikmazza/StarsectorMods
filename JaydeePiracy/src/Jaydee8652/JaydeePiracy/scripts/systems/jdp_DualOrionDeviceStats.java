 ...
 */
public class jdp_DualOrionDeviceStats extends BaseShipSystemScript {
	...
	public static class PusherPlateStateLeft {
	...
	public static class PusherPlateStateRight {
		public float compression;
		public float vel;
		public List<PusherPlateImpulse> impulses = new ArrayList<PusherPlateImpulse>();
	...
	public static class OrionDeviceParams {
		...
		public float pusherPlateMaxOffset = 8f;
		public float pusherPlateMaxOffsetLeft = -8f;
		public float pusherPlateMaxOffsetRight = 8f;
	...
	protected PusherPlateStateLeft pusherStateLeft = new PusherPlateStateLeft();
	protected PusherPlateStateRight pusherStateRight = new PusherPlateStateRight();


	public jdp_DualOrionDeviceStats() {
	...
	protected boolean deployedBombLeft = false;
	protected boolean deployedBombRight = false;
	...
	public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
		...
		Vector2f offsetLeft = new Vector2f(p.pusherPlateMaxOffset, p.pusherPlateMaxOffsetLeft);
		offsetLeft.scale(pusherStateLeft.compression);

		Vector2f offsetRight = new Vector2f(p.pusherPlateMaxOffset, p.pusherPlateMaxOffsetRight);
		offsetRight.scale(pusherStateRight.compression);

			//FORWARDS
		if (!(ship.getEngineController().isTurningLeft() || ship.getEngineController().isTurningRight() /*|| ship.getEngineController().isStrafingLeft() || ship.getEngineController().isStrafingRight()*/)) {
			if (effectLevel >= 1 && !(deployedBombLeft && deployedBombRight)) {
				for (WeaponSlotAPI slot : ship.getHullSpec().getAllWeaponSlotsCopy()) {
					if (slot.getId().equals("BOMB_LEFT")) {
						spawnBomb(ship, slot, "left");
					}
					if (slot.getId().equals("BOMB_RIGHT")) {
						spawnBomb(ship, slot, "right");
					}
				}
				deployedBombLeft = true;
				deployedBombRight = true;

			} else if (state == State.COOLDOWN) {
				deployedBombLeft = false;
				deployedBombRight = false;

			}

			//LEFT
		} else if (ship.getEngineController().isTurningLeft() /*|| ship.getEngineController().isStrafingLeft()*/) {
			if (effectLevel >= 1 && !deployedBombRight) {
				for (WeaponSlotAPI slot : ship.getHullSpec().getAllWeaponSlotsCopy()) {
					if (slot.getId().equals("BOMB_RIGHT")) {
						spawnBomb(ship, slot, "right");
					}
			...
			if (effectLevel >= 1 && !deployedBombLeft) {
				for (WeaponSlotAPI slot : ship.getHullSpec().getAllWeaponSlotsCopy()) {
					if (slot.getId().equals("BOMB_LEFT")) {
						spawnBomb(ship, slot, "left");
					}
		...
		pusherStateLeft.advance(amount);
		pusherStateRight.advance(amount);

		for (WeaponAPI w : ship.getAllWeapons()) {
			if (w.getSlot().getId().equals("PLATE_LEFT")) {
				w.setRenderOffsetForDecorativeBeamWeaponsOnly(offsetLeft);
			}
			if (w.getSlot().getId().equals("PLATE_RIGHT")) {
				w.setRenderOffsetForDecorativeBeamWeaponsOnly(offsetRight);
			}
		}
	}
	...
	public void spawnBomb(ShipAPI source, WeaponSlotAPI slot, String side) {
		...
		Global.getCombatEngine().addPlugin(createBombImpactPlugin(source, slot, bomb, loc, angle, side));
	}
	...
	protected EveryFrameCombatPlugin createBombImpactPlugin(final ShipAPI ship, final WeaponSlotAPI launchSlot,
			final MissileAPI bomb, final Vector2f launchLoc, final float launchAngle, final String side) {
		...
		return new BaseEveryFrameCombatPlugin() {
			...
			public void advance(float amount, List<InputEventAPI> events) {
				...
				if (bomb.isFizzling()) {
					if (!triggered) {
						if (side.equals("left")) {
							pusherStateLeft.addImpulse(p.pusherPlateImpulseForce, p.pusherPlateImpulseDuration);
							ship.setAngularVelocity(ship.getAngularVelocity()-10f);
						};
						if (side.equals("right")) {
							pusherStateRight.addImpulse(p.pusherPlateImpulseForce, p.pusherPlateImpulseDuration);
							ship.setAngularVelocity(ship.getAngularVelocity()+10f);
						};
