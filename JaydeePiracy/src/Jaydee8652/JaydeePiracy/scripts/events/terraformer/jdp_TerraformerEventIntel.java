package Jaydee8652.JaydeePiracy.scripts.events.terraformer;
...
public class jdp_TerraformerEventIntel extends BaseEventIntel implements FleetEventListener,
...
//	public static int PROGRESS_3 = 700;
	...
	public static void addFactorCreateIfNecessary(EventFactor factor, InteractionDialogAPI dialog) {
		if (get() == null) {
			...
			new jdp_TerraformerEventIntel(null, false);
		}
		if (get() != null) {
	...
	public static jdp_TerraformerEventIntel get() {
		return (jdp_TerraformerEventIntel) Global.getSector().getMemoryWithoutUpdate().get(KEY);
	}
	
	
	protected TimeoutTracker<RecentTopographyReadings> recent = new TimeoutTracker<RecentTopographyReadings>();

	public jdp_TerraformerEventIntel(TextPanelAPI text, boolean withIntelNotification) {
		super();
...
//		}
	...
	public void addStageDesc(TooltipMakerAPI info, Object stageId, float initPad, boolean forTooltip) {
		...
		Color h = Misc.getHighlightColor();
		if (stageId == Stage.START) {
			info.addPara("TEST Detailed sensor readings greatly aid hyperspace navigation. "
