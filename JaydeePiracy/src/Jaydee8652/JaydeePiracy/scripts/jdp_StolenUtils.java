...
public class jdp_StolenUtils {
    ...
    public static boolean playerShip(ShipAPI ship) {
        return Global.getCurrentState() == GameState.COMBAT && Global.getCombatEngine() != null && Global.getCombatEngine().getPlayerShip() == ship;
    }

    public static boolean isShip(CombatEntityAPI entity) {
        Class<? extends CombatEntityAPI> entityClass = entity.getClass();
        return  (entityClass.getName().contains("com.fs.starfarer.combat.entities.Ship"));
    }

    public static CombatFleetManagerAPI.AssignmentInfo shipAssignment(ShipAPI ship) {
        ...
        return taskManager.getAssignmentFor(ship);
    }

    public static Boolean escortCheck(ShipAPI ship, ShipAPI target) {
        CombatFleetManagerAPI.AssignmentInfo shipAssignment = shipAssignment(ship);

        if (shipAssignment != null && shipAssignment.getTarget() != null && (shipAssignment.getTarget().getLocation() == target.getLocation()) && (shipAssignment.getTarget().getVelocity() == target.getVelocity())) {
            CombatAssignmentType type = shipAssignment.getType();
            return (shipAssignment.getType() == CombatAssignmentType.LIGHT_ESCORT || shipAssignment.getType() == CombatAssignmentType.MEDIUM_ESCORT || shipAssignment.getType() == CombatAssignmentType.HEAVY_ESCORT);
        }
        return false;
    }
