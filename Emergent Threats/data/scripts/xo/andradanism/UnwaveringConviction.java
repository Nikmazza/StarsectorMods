...
public class UnwaveringConviction extends SCBaseSkillPlugin {
	...
	private static float FLUX_THRESHOLD = 50f;
	private static float ENERGY_ROF_BONUS_LOW = 10f;
    ...
    public void advanceInCombat(SCData data, ShipAPI ship, Float amount) {
		...
		if (ship.getFluxLevel() >= FLUX_THRESHOLD * 0.01f) {
			ship.getMutableStats().getEnergyRoFMult().modifyPercent(ID, ENERGY_ROF_BONUS_HIGH);
		}
		else ship.getMutableStats().getEnergyRoFMult().modifyPercent(ID, ENERGY_ROF_BONUS_LOW);
	}
}