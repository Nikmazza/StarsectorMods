...
public class ASF_KufikiriWeaponScript implements EveryFrameWeaponEffectPlugin {
        ...
        for (DamagingProjectileAPI proj : CombatUtils.getProjectilesWithinRange(weapon.getLocation(), 200f)) {
            if (proj.getWeapon() == weapon && !alreadyRegisteredProjectiles.contains(proj) && engine.isEntityInPlay(proj) && !proj.didDamage()) {
            	if (proj.getDamageType() == DamageType.HIGH_EXPLOSIVE) {
                    engine.addPlugin(new ASF_KufikiriHEProjScript(proj, target));
                    alreadyRegisteredProjectiles.add(proj);
            	} else {
                    engine.addPlugin(new ASF_KufikiriKEProjScript(proj, target));
                    alreadyRegisteredProjectiles.add(proj);
            	}
            }
        }
