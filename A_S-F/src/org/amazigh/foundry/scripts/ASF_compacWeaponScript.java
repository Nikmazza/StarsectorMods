...
public class ASF_compacWeaponScript implements EveryFrameWeaponEffectPlugin {
    ...
    public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine) {
    	...
    	engine.spawnProjectile(source, weapon, "A_S-F_compac_sub", projectile.getLocation(), projectile.getFacing(), source.getVelocity());
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        for (DamagingProjectileAPI proj : CombatUtils.getProjectilesWithinRange(weapon.getLocation(), 200f)) {
            if (proj.getWeapon() == weapon && !alreadyRegisteredProjectiles.contains(proj) && engine.isEntityInPlay(proj) && !proj.didDamage()) {
            	engine.addPlugin(new ASF_compacProjScript(proj, target));
