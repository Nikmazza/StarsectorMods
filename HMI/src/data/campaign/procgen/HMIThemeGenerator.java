...
import data.scripts.world.systems.HMI_mansa;
...
import static data.scripts.world.systems.HMI_mansa.addMessRemnantStationInteractionConfig;
...
public class HMIThemeGenerator extends BaseThemeGenerator {
	...
	public static String  HMISEELE_SYSTEM_KEY = "$hmi_hmiseeleSystem";
	public static String  HMINIGHTMARE_KEY = "$hmi_nightmarestation";

	public static String  HMIMANSA_SYSTEM_KEY = "$hmi_hmimansaSystem";
...
//		}
		...
		BlackLuddStationFleetManager BlackLuddFleets = new BlackLuddStationFleetManager(
				station, 1.0f, 0, maxFleets, 15.0f, 20, 50);
...
//				}
	...
	protected void addSeele(ThemeGenContext context) {
		...
		for (Constellation c : context.constellations) {
			for (StarSystemAPI system : c.getSystems()) {
				...
				if (((loc.x * loc.x) + (loc.y * loc.y)) < (minRadius * minRadius)) {
					continue; //will want to do additional filtering here
				}
		...
		if (sorted.isEmpty()) {
			if (DEBUG) System.out.println("FAILED TO FIND SUITABLE SYSTEM FOR SEELE");
			return;
		}
		Collections.sort(sorted, comp);
	...
	protected void setUpseeleSystem(StarSystemAPI system) {
		system.addTag(Tags.THEME_SPECIAL);
		system.addTag("HMISEELE_SYSTEM_KEY");
		system.addTag(Tags.THEME_UNSAFE);

		Global.getSector().getPersistentData().put(HMISEELE_SYSTEM_KEY, system);
		Global.getSector().getMemoryWithoutUpdate().set(HMISEELE_SYSTEM_KEY, system.getId());
		...
		PlanetAPI hmi_seele_planet1 = null;
		...
		if (hmi_seele_planet1 == null) {
			...
			float orbitDays2 = orbitRadius2 / (20f + random.nextFloat() * 5f);
			float radius2 = 80f + random.nextFloat() * 50f;
			float angle2 = random.nextFloat() * 360f;
			String type = "irradiated";
			String name2 = "Kiel";
			hmi_seele_planet1 = system.addPlanet(Misc.genUID(), system.getStar(), name2, type, angle2, radius2, orbitRadius2, orbitDays2);
			PlanetConditionGenerator.generateConditionsForPlanet(null, hmi_seele_planet1, system.getAge());
		}
			if (hmi_seele_planet1 == null) {
				if (DEBUG) System.out.println("FAILED TO CREATE SEELE STATION IN SYSTEM");
				return;
			}

			system.addRingBand(hmi_seele_planet1, "misc", "rings_dust0", 256f, 0, new Color(0, 0, 0, 0), 64f, 666, 175f, Terrain.RING, "...it...hurts...");
			system.addRingBand(hmi_seele_planet1, "misc", "rings_dust0", 256f, 0, new Color(0, 0, 0, 0), 64f, 1288, 175f, Terrain.RING, "...the...pain...");
			system.addRingBand(hmi_seele_planet1, "misc", "rings_dust0", 256f, 0, new Color(0, 0, 0, 0), 64f, 1678, 175f, Terrain.RING, "...away...");
			system.addRingBand(hmi_seele_planet1, "misc", "rings_dust0", 256f, 0, new Color(0, 0, 0, 0), 64f, 2265, 175f, Terrain.RING, "...get...");
			system.addRingBand(hmi_seele_planet1, "misc", "rings_dust0", 256f, 0, new Color(0, 0, 0, 0), 64f, 2525, 175f, Terrain.RING, "...forgot...how...");
			system.addRingBand(hmi_seele_planet1, "misc", "rings_dust0", 256f, 0, new Color(0, 0, 0, 0), 64f, 3289, 175f, Terrain.RING, "...help...");
		...
		stationNightmare.setCircularOrbit(hmi_seele_planet1, 100, 600, 65f);
		Misc.setDefenderOverride(stationNightmare, new DefenderDataOverride("hmi_nightmare", 1f, 100, 400, 10));

		system.getStar().setName("Seele");
		system.setName("Seele");
	...
	protected void addMansa(ThemeGenContext context) {
		if (DEBUG) System.out.println("Looking for Seele system");
...
//				}
		...
		if (sorted.isEmpty()) {
			if (DEBUG) System.out.println("FAILED TO FIND SUITABLE SYSTEM FOR MANSA");
			return;
		}
		Collections.sort(sorted, comp);
		...
		if (DEBUG) System.out.println("Adding Mansa to [" + system.getName() + "] at [" + system.getLocation() + "]");
		setUpMansaSystem(system);


		if (DEBUG) System.out.println("Finished adding MANSA system\n\n\n\n\n");
	}
	...
	protected void setUpMansaSystem(StarSystemAPI system) {
		...
		Global.getSector().getPersistentData().put(HMIMANSA_SYSTEM_KEY, system);
		Global.getSector().getMemoryWithoutUpdate().set(HMIMANSA_SYSTEM_KEY, system.getId());
		...
		if (mansa2 == null) {
			...
			String type = "barren";
			String name2 = namePick.nameWithRomanSuffixIfAny;
			mansa2 = system.addPlanet(Misc.genUID(), system.getStar(), name2, type, angle2, radius2, orbitRadius2, orbitDays2);
		}
		if (mansa2 == null) {
			return;
		}
		mansa2.changeType(Planets.BARREN, random);
		mansa2.getMarket().getConditions().clear();
		PlanetConditionGenerator.generateConditionsForPlanet(null, mansa2, system.getAge());
		...
		system.addRingBand(system.getStar(), "misc", "rings_dust0", 256f, 0, Color.white, 256f, mansa2.getCircularOrbitRadius(), mansa2.getCircularOrbitPeriod(), Terrain.RING, "Khalifa Remnant");
		system.addAsteroidBelt(system.getStar(), 240, mansa2.getCircularOrbitRadius(), 128, mansa2.getCircularOrbitPeriod(), mansa2.getCircularOrbitPeriod()+100f, Terrain.ASTEROID_BELT, null);
		...
		fleet1.getMemoryWithoutUpdate().set(MemFlags.FLEET_INTERACTION_DIALOG_CONFIG_OVERRIDE_GEN,
				new HMI_mansa.MessStationFIDConfig());

		if (fleet1 == null) {
			if (DEBUG) System.out.println("FAILED TO CREATE MANSA STATION IN SYSTEM");
			return;
		}
		...
		beacon.setCircularOrbitPointingDown(anchor, 100, 200, 65f);
		...
		beacon.getMemoryWithoutUpdate().set(WarningBeaconEntityPlugin.PING_COLOR_KEY, new Color(196, 0, 250,255));
		beacon.getMemoryWithoutUpdate().set(WarningBeaconEntityPlugin.GLOW_COLOR_KEY, new Color(150, 0, 250, 189));
