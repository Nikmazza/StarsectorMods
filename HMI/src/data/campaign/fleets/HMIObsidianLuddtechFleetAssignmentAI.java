...
public class HMIObsidianLuddtechFleetAssignmentAI extends RouteFleetAssignmentAI {
	...
	protected void doPiracyCheck() {
		...
		boolean isCurrentlyPirate = fleet.getFaction().getId().equals(Factions.LUDDIC_PATH);
		...
		if (visible.size() == 1) {
			...
			for (CampaignFleetAPI other : visible) {
				if (fleet.getAI() != null && 
						Global.getSector().getFaction(Factions.LUDDIC_PATH).isHostileTo(other.getFaction())
						&& !other.getTags().equals("ObsidianLuddTech")
						&& !other.getFaction().equals(Factions.LUDDIC_CHURCH)) {
