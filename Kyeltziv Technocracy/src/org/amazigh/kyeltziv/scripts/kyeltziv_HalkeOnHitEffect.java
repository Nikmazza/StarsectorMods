...
public class kyeltziv_HalkeOnHitEffect implements OnHitEffectPlugin {

	public static float DAMAGE = 120;

	public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target,
					  Vector2f point, boolean shieldHit, ApplyDamageResultAPI damageResult, CombatEngineAPI engine) {
		if (!shieldHit && target instanceof ShipAPI) {
			DAMAGE = projectile.getDamageAmount() * 0.15f;
			...
			for (int i=0; i < 25; i++) {
				...
				for (int j=0; j < 4; j++) {
					Vector2f sparkVel1 = MathUtils.getPointOnCircumference(target.getVelocity(), MathUtils.getRandomNumberInRange(10f, 135f), projectile.getFacing() + MathUtils.getRandomNumberInRange(120f, 240f));
					...
					Global.getCombatEngine().addSmoothParticle(point,
							...
							new Color(240,140,100,255));
				}
				
				Vector2f sparkVel2 = MathUtils.getPointOnCircumference(target.getVelocity(), MathUtils.getRandomNumberInRange(80f, 225f), projectile.getFacing() + MathUtils.getRandomNumberInRange(176f, 184f));
			...
			Global.getCombatEngine().spawnExplosion(point, target.getVelocity(), new Color(210,120,255,255), 12f, 0.7f);
		}
	}
