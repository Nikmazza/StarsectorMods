...
public class bt_holytorp_aura extends BaseEveryFrameCombatPlugin {
    ...
    public bt_holytorp_aura(DamagingProjectileAPI proj) {
        ...
        // Logging from previous version (can be kept or removed as needed)
    ...
    public void advance(float amount, List<InputEventAPI> events) {
        ...
        // --- Glint Effect (Only when armed) ---
        if (isArmedState && random.nextFloat() < (isArmedState ? 0.5f : 0.1f) ) { // Higher chance of glint when armed
            if (flareSpriteMain != null) {
                ...
                Vector2f glintLoc = MathUtils.getRandomPointInCircle(projLoc, collisionR > 0 ? collisionR * 0.3f : 6f); // Slightly larger glint area
